...
# Configure Redis server address via ConfigMap
# For E2E tests, ALL Argo CD instances (including control-plane) should connect to their local Redis
# The principal's Redis proxy is used by agents for Redis communication, not by control-plane Argo CD
echo "Setting redis.server to local argocd-redis in argocd-cmd-params-cm..."
