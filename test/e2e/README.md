...
**Note:** The E2E test fixture uses `InsecureSkipVerify: true` for Redis connections to simplify testing across different environments. The principal and agents themselves use proper TLS validation when connecting via localhost port-forwards (which match the certificate SANs). TLS encryption is fully enabled in all scenarios.
